...
class Panelcontroller
{
    ...
    public function usergetthreeall($data, $conn)
    {
        $userprivate = $data->private;
        ...
        $stmt = $conn->prepare("
        SELECT u.number, u.value, IFNULL(a.status, 'unknown') AS status
        FROM user_three_limit u
        LEFT JOIN agent_limit a 
            ON a.agentprivate = u.agentprivate 
            AND a.number = u.number 
            AND a.value = u.value
        WHERE u.agentprivate = ? AND u.userprivate = ?
        ORDER BY u.number ASC
    ");

        $stmt->bind_param('ss', $agentprivate, $userprivate);
        $stmt->execute();
    ...
    public function usergetthree($data, $conn)
    {
        ...
        $mm = isset($pageNames[$page]) ? $pageNames[$page] : "unknown";
